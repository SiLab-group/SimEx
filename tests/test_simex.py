...
def test_modifier():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        logger = Logger(
            filename=os.path.join(temp_dir, "test.log"), simex_settings=settings
        )
...
def test_simulator_controller():
    ...
    mod_x, sim_y_list = SimulatorController.simulate(mod_x_list, Simulator.sim_func_A)
...
def test_simulator_parallel():
    ...
    mod_x, sim_y_list = SimulatorController.simulate_parallel(
        mod_x_list, Simulator.sim_func_C_no_noise, workers=8
    )
...
def test_simulator_values():
    ...
    mod_x, sim_y_list = SimulatorController.simulate(
        mod_x_list, Simulator.sim_func_C_no_noise
    )
    assert len(mod_x) == len(sim_y_list)
    assert sim_y_list == [
        1733.8407448206267,
