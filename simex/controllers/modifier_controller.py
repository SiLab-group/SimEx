...
class ModifierController:
    ...
    def control(self, intervals_list, selected_modifier, do_plot):
        ...
        if self.settings.modifier_data_point < self.settings.modifier_incremental_unit:
            if (
                self.settings.extensive_search is True
                and self.settings.extensive_iteration is False
            ):
        ...
        mod_ticks = np.arange(
            ...
            self.settings.modifier_data_point,
        )
        for _, (interval_min_tick, interval_max_tick) in enumerate(intervals_list):
            ...
            print(
                "[MODC]: (interval_min_tick, interval_max_tick): ",
                (interval_min_tick, interval_max_tick),
            )

            # Extract mod_ticks that are within intervals_list
            mod_filtered_ticks = [
                tick
                for tick in mod_ticks
                if interval_min_tick < tick < interval_max_tick
            ]
            # If not empty
            if np.any(mod_filtered_ticks):
                ...
                # Fixed in numpy 2.0
                mod_x = selected_modifier(
                    mod_filtered_ticks,
                    new_min=np.min(np.array(mod_filtered_ticks, dtype=np.int64)),
                ...
                if self.settings.add_first_last_points:
                    new_mod_x = (
                        [float(interval_min_tick)] + mod_x + [float(interval_max_tick)]
                    )
                    all_intervals_mod.append(new_mod_x)
                else:
                    all_intervals_mod.append(mod_x)

        current_iteration_points_number = sum(
            len(sublist) for sublist in all_intervals_mod
        )
        ...
        logger_modifier_arguments["current_iteration_points_number"] = (
        ...
        self.settings.modifier_data_point = (
            self.settings.modifier_data_point - self.settings.modifier_incremental_unit
        )
