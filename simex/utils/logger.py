...
import numpy as np
from simex.config.settings import lgs, timestamp
...
class Logger:
    ...
    def log_main(self, logger_arguments):
        ...
        if logger_arguments["log_contex"] == "Overall Stats" and logger_arguments["main_status"] == "end cycle":
            ...
            message = "MOD - Total generated points: " + \
                      str(self.settings.mgs_points_generated_total)
            self._write_log('[MAIN]: ', message)
            message = "MOD - Total intervals used for points generation: " + \
                      str(self.settings.mgs_points_generation_intervals)
            self._write_log('[MAIN]: ', message)
    ...
    def log_modifier(self, logger_arguments):
        ...
        if logger_arguments["log_contex"] == "internal MOD stats":
            ...
            if lgs["log_granularity"] > 0:
                message = "Iteration " + str(self.settings.mgs_mod_iterations) + " has generated " + str(
    ...
    def _plot_results_tailing(self, all_fit_intervals_data, remaining_unfit_intervals):
        ...
        funcs_values = []
        # Create x values
        x = np.linspace(self.settings.domain_min_interval, self.settings.domain_max_interval,
                        self.settings.ops_number_x_points, dtype=np.float64)
        ...
        def transition(xx, xx_conn, width=self.settings.ops_sigmoid_width):
            return 1.0 / (1.0 + np.exp(-2 / width * (xx - xx_conn)))
        ...
        for i in range(0, len(funcs)):
            ...
            if funcs[i].name in colors.keys():
                ...
                if self.settings.ops_threshold_plot:
                    ax.plot(funcs_values[i].x_values, funcs_values[i].y_values + self.settings.vfs_threshold_y_fitting,
                            'm--', linewidth=2)
                    ax.plot(funcs_values[i].x_values, funcs_values[i].y_values - self.settings.vfs_threshold_y_fitting,
                            'm--', linewidth=2)
            else:
        ...
        # Save the smoothened graph
        smooth = {'x': x, 'y': y, 'connection_points': connection_points}
        save_object(smooth, os.path.join(self.settings.results_dir, f"smooth-{self.settings.instance_name}-{self.timestamp}.pkl"))
        ...
        if self.settings.ops_predicted_points:
            ...
            smooth['original_points'] = [x_point, y_point]
        plt.xlabel(self.settings.ops_x_labels)
        plt.ylabel(self.settings.ops_y_labels)
        plt.title(self.settings.ops_title)
        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
