...
import numpy as np
from ..core.settings import timestamp
...
class Validator:
    ...
    def fit_curve(self, x_values, y_values):
        ...
        while degree <= max_deg:
            ...
            current_mse = mean_squared_error(
                y_values, current_y_pred
            ) + penality_weight * np.sum(current_coeff[:-1] ** 2)
    ...
    def find_unfit_points(self, x_values, y_values, fitted_curve):
        ...
        unfit_indices = np.where(
            np.abs(residuals) > self.settings.vfs_threshold_y_fitting
        )[0]
        ...
        for i, point in enumerate(x_values):
            ...
            if np.round(point, 4) not in np.round(unfit_point_x, 4):
                ...
                current_interval.append(
                    x_values[i - 1]
                    + self.settings.vfs_threshold_x_interval * interpoint_interval
                )
    ...
    def find_fit_points(self, x_values_all, y_values_all, unfit_points, tolerance=1e-5):
        ...
        rest_of_points = [
            (x, y)
            for x, y in zip(x_values_all, y_values_all)
            if all(
                abs(x - xp) > tolerance or abs(y - yp) > tolerance
                for xp, yp in unfit_points
            )
        ]
    ...
    def get_fit_intervals(
        self, unfit_x_interval, domain_min_interval, domain_max_interval
    ):
        ...
        for current_interval, next_interval in zip(
            unfit_x_interval, unfit_x_interval[1:]
        ):
        ...
        if unfit_x_interval[-1][1] < domain_max_interval:
            fit_x_intervals.append([unfit_x_interval[-1][1], domain_max_interval])
    ...
    def local_exploration_validator_A(self, x_values, y_values, selected_interval=0):
        ...
        fit_interval = self.get_fit_intervals(
            unfit_interval,
            domain_min_interval=selected_interval[0],
            domain_max_interval=selected_interval[1],
        )
        ...
        for _, interval in enumerate(fit_interval):
            ...
            logger_validator_arguments = {
                "log_contex": "fit_VAL_stats",
                "fit_interval": interval,
                "fitting_function": equation,
                "fit_points": filtered_fit_points,
            }
            self.logger.log_validator(logger_validator_arguments)

        # print(unfit_interval)
        self.plot_curve(
            x_values, y_values, fitted_curve, unfit_interval, predicted_values
        )
    ...
    def plot_curve(
        self, x_values, y_values, fitted_curve, unfit_interval, predicted_values
    ):  # Add self
        import datetime
        ...
        plt.plot(
            fitted_curve[2], fitted_curve[1], color="red", label="Polynomial Regression"
        )
        plt.plot(
            fitted_curve[2],
            fitted_curve[1] + self.settings.vfs_threshold_y_fitting,
            color="black",
            label="threshold ",
        )
        plt.plot(
        ...
        for start, end in unfit_interval:
            ...
            plt.axvspan(
                ...
                color="orange",
                alpha=0.3,
                label=f"Unfit Interval {count}: [{round(start)},{round(end)}]",
            )
        ...
        plt.savefig(
            os.path.join(
                ...
                f"TTS_vs_Volume_{self.settings.instance_name}-{timestamp}.pdf",
            ),
            format="pdf",
        )
        plt.show()
