...
import numpy as np
# Set instance name
os.environ['INSTANCE_NAME'] = 'NOVSL_script'
...
while is_main_func:
    ...
    count += 1
    save_object(validator_controller_novsl, os.path.join(results_dir,f"vc_novsl_loop-{count}-{timestamp}.pkl"))
...
save_object(logger.all_fit_intervals_data,os.path.join(results_dir, f"logger-novsl_script-fitted_intervals-{timestamp}.pkl"))
# If not empty
