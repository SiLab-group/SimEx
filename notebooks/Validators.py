...
class Validators:
    ...
    def find_unfit_points(self, x_values, y_values, fitted_curve):
        ...
        unfit_indices =  np.where(np.abs(residuals) > vfs["threshold_y_fitting"])[0]
    ...
    def find_fit_points(self, x_values_all, y_values_all, unfit_points, tolerance=1e-5):
        ...
        rest_of_points = [(x, y) for x, y in zip(x_values_all, y_values_all) if all(abs(x - xp) > tolerance or abs(y - yp) > tolerance for xp, yp in unfit_points)]
    ...
    def local_exploration_validator_A(self,x_values, y_values, selected_interval=0):
        ...
        unfit_points,predicted_values = self.find_unfit_points(x_values, y_values,fitted_curve=fitted_curve)
        # unfit_interval = self.generate_intervals_from_unfit_points(unfit_points,threshold=0.75 )        # unfit_points = self.find_unfit_points(x_values, y_values, fitted_curve, threshold=threshold)
        unfit_interval = self.generate_intervals_from_unfit_points(unfit_points,x_values)
        ...
        for _ , interval in enumerate(fit_interval):
            ...
            filtered_fit_points = [(round(point[0], 5), round(point[1], 5)) for point in fit_points if interval[0] <= point[0] <= interval[1]]
        ...
        # self.save_to_text_file('output.txt', unfit_points, unfit_interval,x_values)
